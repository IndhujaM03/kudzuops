import os
import json
from datetime import datetime, date, date, date
from typing import Any, Dict, List, Optional

import psycopg
from fastapi import APIRouter, HTTPException, Request, Depends, Depends, Depends
                        ...
                        if isinstance(cv_list, list):
                            for cv in cv_list:
                                if cv.get('cv_url') and not cv['cv_url'].startswith('http'):
                                    ...
                                    cv['cv_url'] = f"{os.getenv('API_BASE_URL', '{API_BASE_URL}')}/cv-file/{row['recruiter_id']}/{row['demand_id']}/{encoded_filename}" if API_BASE_URL else None
                ...
                for row in result:
                    if row.get('cv_list'):
                        ...
                        if isinstance(cv_list, list):
                            ...
                            # Only include the row if there are unprocessed CVs
                            if filtered_cv_list:
                                row['cv_list'] = filtered_filtered_cv_list
