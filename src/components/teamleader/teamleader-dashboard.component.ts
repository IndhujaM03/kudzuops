  ...
  template: `
    <div class="dashboard-container">
      ...
      <div class="dashboard-header">
        <h1 class="dashboard-title">Team Leader Report – Comprehensive Analysis of Recruitment Performance</h1>
      </div>
      ...
      <div class="key-highlights">
        ...
        <div class="highlight-card">
          <div class="card-icon demand-icon">📋</div>
          <div class="card-content">
            <div class="card-label">Current Demand</div>
            <div class="card-value">{{ currentDemand() }}</div>
          </div>
        </div>
        <div class="highlight-card">
          <div class="card-icon recruiters-icon">👥</div>
          <div class="card-content">
            <div class="card-label">Number of Recruiters</div>
            <div class="card-value">{{ numberOfRecruiters() }}</div>
          </div>
        </div>
      </div>
      ...
      <div class="charts-grid">
        ...
        <div class="chart-card">
          <div class="chart-header">
            <h3 class="chart-title">Distribution of Demand by Recruiters</h3>
            <div class="chart-filters">
              <div class="filter-container">
                <div class="filter-group-view">
                  <label class="filter-label">View:</label>
                  <div class="radio-group">
                    <label class="radio-label">
                      <input type="radio" name="demandRecruitersView" value="daily" [(ngModel)]="demandRecruitersView" (ngModelChange)="onDemandRecruitersFilterChange()">
                      <span>Daily</span>
                    </label>
                    <label class="radio-label">
                      <input type="radio" name="demandRecruitersView" value="weekly" [(ngModel)]="demandRecruitersView" (ngModelChange)="onDemandRecruitersFilterChange()">
                      <span>Weekly</span>
                    </label>
                    <label class="radio-label">
                      <input type="radio" name="demandRecruitersView" value="monthly" [(ngModel)]="demandRecruitersView" (ngModelChange)="onDemandRecruitersFilterChange()">
                      <span>Monthly</span>
                    </label>
                  </div>
                </div>
                <div class="filter-group-month">
                  <label class="filter-label">Month:</label>
                  <select class="filter-dropdown-month" [(ngModel)]="demandRecruitersMonth" (ngModelChange)="onDemandRecruitersFilterChange()">
                    <option *ngFor="let option of monthOptions" [value]="option.value">{{ option.label }}</option>
                  </select>
                </div>
              </div>
          ...
          <div id="demand-recruiters-chart" class="chart-container"></div>
        </div>
        ...
        <div class="chart-card chart-card-full">
          <div class="chart-header">
            <h3 class="chart-title">Submissions by Recruiters</h3>
            <div class="chart-filters">
              <div class="filter-container">
                <div class="filter-group-view">
                  <label class="filter-label">View:</label>
                  <div class="radio-group">
                    <label class="radio-label">
                      <input type="radio" name="submissionsRecruitersView" value="daily" [(ngModel)]="submissionsRecruitersView" (ngModelChange)="onSubmissionsRecruitersFilterChange()">
                      <span>Daily</span>
                    </label>
                    <label class="radio-label">
                      <input type="radio" name="submissionsRecruitersView" value="weekly" [(ngModel)]="submissionsRecruitersView" (ngModelChange)="onSubmissionsRecruitersFilterChange()">
                      <span>Weekly</span>
                    </label>
                    <label class="radio-label">
                      <input type="radio" name="submissionsRecruitersView" value="monthly" [(ngModel)]="submissionsRecruitersView" (ngModelChange)="onSubmissionsRecruitersFilterChange()">
                      <span>Monthly</span>
                    </label>
                  </div>
                </div>
                <div class="filter-group-month">
                  <label class="filter-label">Month:</label>
                  <select class="filter-dropdown-month" [(ngModel)]="submissionsRecruitersMonth" (ngModelChange)="onSubmissionsRecruitersFilterChange()">
                    <option *ngFor="let option of monthOptions" [value]="option.value">{{ option.label }}</option>
                  </select>
                </div>
              </div>
          ...
          <div id="submissions-recruiters-chart" class="chart-container"></div>
        </div>
...
export class TeamLeaderDashboardComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  loadDailyTrendChart(): void {
    ...
    this.teamLeaderService.getDailySubmissionsTrend(
      dateRange.startDate,
      dateRange.endDate
    ).subscribe({
      next: (response) => {
  ...
  loadDemandByRecruitersChart(): void {
    const dateRange = this.getDateRange(this.demandRecruitersView, this.demandRecruitersMonth);
    this.teamLeaderService.getDemandByRecruiters(
      dateRange.startDate,
      dateRange.endDate
    ).subscribe({
      next: (response) => {
        const data = response.distribution || [];
        this.renderPieChart('demand-recruiters-chart', data, 'recruiter_name', 'count', 'percentage', true);
      },
      error: (error) => {
        console.error('Error loading demand by recruiters:', error);
        this.showEmptyState('demand-recruiters-chart', 'No demand data available');
      }
    });
  }
  ...
  loadDemandBySpocsChart(): void {
    ...
    this.teamLeaderService.getDemandBySpocs(
      dateRange.startDate,
      dateRange.endDate
    ).subscribe({
      next: (response) => {
  ...
  loadDemandByStatusChart(): void {
    ...
    this.teamLeaderService.getDemandByStatus(
      dateRange.startDate,
      dateRange.endDate
    ).subscribe({
      next: (response) => {
  ...
  loadDemandBySkillChart(): void {
    ...
    this.teamLeaderService.getDemandBySkill(
      dateRange.startDate,
      dateRange.endDate
    ).subscribe({
      next: (response) => {
  ...
  loadSubmissionsByRecruitersChart(): void {
    const dateRange = this.getDateRange(this.submissionsRecruitersView, this.submissionsRecruitersMonth);
    this.teamLeaderService.getSubmissionsByRecruiters(
      dateRange.startDate,
      dateRange.endDate
    ).subscribe({
      next: (response) => {
        const data = response.recruiter_submissions || [];
        this.renderBarChart('submissions-recruiters-chart', data, 'recruiter_name', 'count', 'Recruiter', 'Submissions');
      },
      error: (error) => {
        console.error('Error loading submissions by recruiters:', error);
        this.showEmptyState('submissions-recruiters-chart', 'No submission data available');
      }
    });
  }
  ...
  loadSubmissionsBySpocsChart(): void {
    ...
    this.teamLeaderService.getSubmissionsBySpocs(
      dateRange.startDate,
      dateRange.endDate
    ).subscribe({
      next: (response) => {
  ...
  onDemandRecruitersFilterChange(): void {
    this.loadDemandByRecruitersChart();
  }
  ...
  onSubmissionsRecruitersFilterChange(): void {
    this.loadSubmissionsByRecruitersChart();
  }
  ...
  renderLineChart(containerId: string, data: Array<{ date: string; count: number; recruiters?: Array<{ recruiter_name: string; count: number }> }>): void {
    ...
    g.selectAll('.dot')
      ...
      .on('mouseover', (event, d) => {
        const tooltipContent = `
          ...
          ${d.recruiters && d.recruiters.length > 0 ? d.recruiters.map(r => `<div class="tooltip-item">${r.recruiter_name}: ${r.count}</div>`).join('') : ''}
        `;
  ...
  renderBarChart(containerId: string, data: any[], xKey: string, yKey: string, xLabel: string, yLabel: string): void {
    ...
    const getBarColor = (d: any, index: number): string => {
      ...
      // Use recruiterChartColors for submissions by recruiters chart
      if (containerId === 'submissions-recruiters-chart') {
