  ...
  template: `
    <div class="dashboard-container">
      ...
      <div class="dashboard-header">
        <h1 class="dashboard-title">Manager Report – Comprehensive Analysis of Recruitment Performance</h1>
      </div>
      ...
      <div class="key-highlights">
        ...
        <div class="highlight-card">
          <div class="card-icon team-leaders-icon">👥</div>
          <div class="card-content">
            <div class="card-label">Number of Team Leaders</div>
            <div class="card-value">{{ numberOfTeamLeaders() }}</div>
          </div>
        </div>
      </div>
      ...
      <div class="charts-grid">
        ...
        <!-- Demand by Team Leaders (Pie) -->
        <div class="chart-card">
          <div class="chart-header">
            <h3 class="chart-title">Distribution of Demand by Team Leaders</h3>
            <div class="chart-filters">
              <div class="filter-container">
                <div class="filter-group-view">
                  <label class="filter-label">View:</label>
                  <div class="radio-group">
                    <label class="radio-label">
                      <input type="radio" name="demandTeamLeadersView" value="daily" [(ngModel)]="demandTeamLeadersView" (ngModelChange)="onDemandTeamLeadersFilterChange()">
                      <span>Daily</span>
                    </label>
                    <label class="radio-label">
                      <input type="radio" name="demandTeamLeadersView" value="weekly" [(ngModel)]="demandTeamLeadersView" (ngModelChange)="onDemandTeamLeadersFilterChange()">
                      <span>Weekly</span>
                    </label>
                    <label class="radio-label">
                      <input type="radio" name="demandTeamLeadersView" value="monthly" [(ngModel)]="demandTeamLeadersView" (ngModelChange)="onDemandTeamLeadersFilterChange()">
                      <span>Monthly</span>
                    </label>
                  </div>
                </div>
                <div class="filter-group-month">
                  <label class="filter-label">Month:</label>
                  <select class="filter-dropdown-month" [(ngModel)]="demandTeamLeadersMonth" (ngModelChange)="onDemandTeamLeadersFilterChange()">
                    <option *ngFor="let option of monthOptions" [value]="option.value">{{ option.label }}</option>
                  </select>
                </div>
              </div>
          ...
          <div id="demand-team-leaders-chart" class="chart-container"></div>
        </div>
        ...
        <!-- Submissions by Team Leaders (Bar) -->
        <div class="chart-card chart-card-full">
          <div class="chart-header">
            <h3 class="chart-title">Submissions by Team Leaders</h3>
            <div class="chart-filters">
              <div class="filter-container">
                <div class="filter-group-view">
                  <label class="filter-label">View:</label>
                  <div class="radio-group">
                    <label class="radio-label">
                      <input type="radio" name="submissionsTeamLeadersView" value="daily" [(ngModel)]="submissionsTeamLeadersView" (ngModelChange)="onSubmissionsTeamLeadersFilterChange()">
                      <span>Daily</span>
                    </label>
                    <label class="radio-label">
                      <input type="radio" name="submissionsTeamLeadersView" value="weekly" [(ngModel)]="submissionsTeamLeadersView" (ngModelChange)="onSubmissionsTeamLeadersFilterChange()">
                      <span>Weekly</span>
                    </label>
                    <label class="radio-label">
                      <input type="radio" name="submissionsTeamLeadersView" value="monthly" [(ngModel)]="submissionsTeamLeadersView" (ngModelChange)="onSubmissionsTeamLeadersFilterChange()">
                      <span>Monthly</span>
                    </label>
                  </div>
                </div>
                <div class="filter-group-month">
                  <label class="filter-label">Month:</label>
                  <select class="filter-dropdown-month" [(ngModel)]="submissionsTeamLeadersMonth" (ngModelChange)="onSubmissionsTeamLeadersFilterChange()">
                    <option *ngFor="let option of monthOptions" [value]="option.value">{{ option.label }}</option>
                  </select>
                </div>
              </div>
          ...
          <div id="submissions-team-leaders-chart" class="chart-container"></div>
        </div>
...
export class ManagerDashboardComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  loadDashboardData(): void {
    ...
    this.managerService.getKeyHighlights().subscribe({
      next: (data) => {
        this.totalSubmissions.set(data.total_submissions || 0);
        this.currentDemand.set(data.current_demand || 0);
        this.numberOfTeamLeaders.set(data.number_of_team_leaders || 0);
      },
  ...
  loadDailyTrendChart(): void {
    ...
    this.managerService.getDailySubmissionsTrend(
      dateRange.startDate,
      dateRange.endDate
    ).subscribe({
      next: (response) => {
  ...
  loadDemandByTeamLeadersChart(): void {
    const dateRange = this.getDateRange(this.demandTeamLeadersView, this.demandTeamLeadersMonth);
    this.managerService.getDemandByTeamLeaders(
      dateRange.startDate,
      dateRange.endDate
    ).subscribe({
      next: (response) => {
        const data = response.distribution || [];
        this.renderPieChart('demand-team-leaders-chart', data, 'team_leader_name', 'count', 'percentage', true);
      },
      error: (error) => {
        console.error('Error loading demand by team leaders:', error);
        this.showEmptyState('demand-team-leaders-chart', 'No demand data available');
      }
    });
  }
  ...
  loadDemandBySpocsChart(): void {
    ...
    this.managerService.getDemandBySpocs(
      dateRange.startDate,
      dateRange.endDate
    ).subscribe({
      next: (response) => {
  ...
  loadDemandByStatusChart(): void {
    ...
    this.managerService.getDemandByStatus(
      dateRange.startDate,
      dateRange.endDate
    ).subscribe({
      next: (response) => {
  ...
  loadDemandBySkillChart(): void {
    ...
    this.managerService.getDemandBySkill(
      dateRange.startDate,
      dateRange.endDate
    ).subscribe({
      next: (response) => {
  ...
  loadSubmissionsByTeamLeadersChart(): void {
    const dateRange = this.getDateRange(this.submissionsTeamLeadersView, this.submissionsTeamLeadersMonth);
    this.managerService.getSubmissionsByTeamLeaders(
      dateRange.startDate,
      dateRange.endDate
    ).subscribe({
      next: (response) => {
        const data = response.team_leader_submissions || [];
        this.renderBarChart('submissions-team-leaders-chart', data, 'team_leader_name', 'count', 'Team Leader', 'Submissions');
      },
      error: (error) => {
        console.error('Error loading submissions by team leaders:', error);
        this.showEmptyState('submissions-team-leaders-chart', 'No submission data available');
      }
    });
  }
  ...
  loadSubmissionsBySpocsChart(): void {
    ...
    this.managerService.getSubmissionsBySpocs(
      dateRange.startDate,
      dateRange.endDate
    ).subscribe({
      next: (response) => {
  ...
  onDemandTeamLeadersFilterChange(): void {
    this.loadDemandByTeamLeadersChart();
  }
  ...
  onSubmissionsTeamLeadersFilterChange(): void {
    this.loadSubmissionsByTeamLeadersChart();
  }
    ...
    g.selectAll('.dot')
      ...
      .on('mouseover', (event, d) => {
        const tooltipContent = `
          ...
          ${d.team_leaders && d.team_leaders.length > 0 ? d.team_leaders.map(r => `<div class="tooltip-item">${r.team_leader_name}: ${r.count}</div>`).join('') : ''}
        `;
