  ...
  template: `
    <div class="dashboard-container">
      ...
      <div class="dashboard-header">
        <h1 class="dashboard-title">Super Admin Report – Comprehensive Analysis of Recruitment Performance</h1>
      </div>

      <!-- Role Count Cards (hide automatically when zero) -->
      <div class="key-highlights">
        <div class="highlight-card" *ngIf="pendingCount() > 0">
          <div class="card-icon managers-icon">⏳</div>
          <div class="card-content">
            <div class="card-label">Pending Approvals</div>
            <div class="card-value">{{ pendingCount() }}</div>
          </div>
        </div>
        <div class="highlight-card" *ngIf="clusterManagersCount() > 0">
          <div class="card-icon managers-icon">🏢</div>
          <div class="card-content">
            <div class="card-label">Cluster Managers</div>
            <div class="card-value">{{ clusterManagersCount() }}</div>
          </div>
        </div>
        <div class="highlight-card" *ngIf="businessHeadsCount() > 0">
          <div class="card-icon managers-icon">👔</div>
          <div class="card-content">
            <div class="card-label">Business Heads</div>
            <div class="card-value">{{ businessHeadsCount() }}</div>
          </div>
        </div>
        <div class="highlight-card" *ngIf="managersCount() > 0">
          <div class="card-icon managers-icon">👥</div>
          <div class="card-content">
            <div class="card-label">Managers</div>
            <div class="card-value">{{ managersCount() }}</div>
          </div>
        </div>
        <div class="highlight-card" *ngIf="teamLeadersCount() > 0">
          <div class="card-icon managers-icon">🧩</div>
          <div class="card-content">
            <div class="card-label">Team Leaders</div>
            <div class="card-value">{{ teamLeadersCount() }}</div>
          </div>
        </div>
        <div class="highlight-card" *ngIf="recruitersCount() > 0">
          <div class="card-icon managers-icon">🔎</div>
          <div class="card-content">
            <div class="card-label">Recruiters</div>
            <div class="card-value">{{ recruitersCount() }}</div>
          </div>
        </div>
      </div>
      ...
      <div class="charts-grid">
        <div class="chart-card chart-card-full">
          <div class="chart-header">
            <h3 class="chart-title">Daily Submissions Trend (Global)</h3>
          ...
          <div id="sa-daily-trend-chart" class="chart-container"></div>
        </div>

        <div class="chart-card">
          <div class="chart-header">
            <h3 class="chart-title">Demand by Managers</h3>
            <div class="chart-filters">
              <div class="filter-container">
                <div class="filter-group-view">
                  <label class="filter-label">View:</label>
          ...
          <div id="sa-demand-managers-chart" class="chart-container"></div>
        </div>
        ...
        <div class="chart-card">
          ...
          <div id="sa-demand-status-chart" class="chart-container"></div>
        </div>
        ...
        <div class="chart-card">
          ...
          <div id="sa-demand-skill-chart" class="chart-container"></div>
        </div>
        ...
        <div class="chart-card chart-card-full">
          ...
          <div id="sa-submissions-managers-chart" class="chart-container"></div>
        </div>

        <div class="chart-card">
          <div class="chart-header">
            <h3 class="chart-title">Demand by SPOCs</h3>
            <div class="chart-filters">
              <div class="filter-container">
                <div class="filter-group-view">
                  <label class="filter-label">View:</label>
          ...
          <div id="sa-demand-spocs-chart" class="chart-container"></div>
        </div>
        ...
        <div class="chart-card">
          ...
          <div id="sa-submissions-spocs-chart" class="chart-container"></div>
        </div>
      </div>
    </div>
...
export class SuperAdminOverviewComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  // Role counts (hide cards when zero)
  pendingCount = signal<number>(0);
  clusterManagersCount = signal<number>(0);
  businessHeadsCount = signal<number>(0);
  managersCount = signal<number>(0);
  teamLeadersCount = signal<number>(0);
  recruitersCount = signal<number>(0);
  ...
  ngOnInit(): void {
    // Load role counts (cards hidden when zero)
    this.superAdminService.getPendingCount().subscribe({ next: (r: any) => this.pendingCount.set(r?.count || 0), error: () => this.pendingCount.set(0) });
    this.loadRoleCount('cluster_manager', this.clusterManagersCount);
    this.loadRoleCount('business_head', this.businessHeadsCount);
    this.loadRoleCount('manager', this.managersCount);
    this.loadRoleCount('team_leader', this.teamLeadersCount);
    this.loadRoleCount('recruiter', this.recruitersCount);
  }
  ...
  private loadKeyHighlights(): void {
    this.superAdminService.getKeyHighlights().subscribe({
      next: (data: any) => {
        console.log('Super Admin Key Highlights Response:', data);
        ...
        console.log('Super Admin Signals updated:', {
      ...
      error: (error: any) => {
        console.error('Error loading Super Admin key highlights:', error);
  ...
  private loadRoleCount(role: string, targetSignal: any): void {
    this.superAdminService.getUsersByRole(role).subscribe({
      next: (list: any[]) => targetSignal.set(Array.isArray(list) ? list.length : 0),
  ...
  onDemandManagersFilterChange(): void { this.loadDemandByManagersChart(); }
  onDemandStatusFilterChange(): void { this.loadDemandByStatusChart(); }
  onDemandSkillFilterChange(): void { this.loadDemandBySkillChart(); }
  onSubmissionsManagersFilterChange(): void { this.loadSubmissionsByManagersChart(); }
  ...
  private loadDailyTrendChart(): void {
    ...
    this.superAdminService.getDailySubmissionsTrend(dateRange.startDate, dateRange.endDate).subscribe({
      next: (res: any) => {
        const data = res.daily_trend || [];
        this.renderLineChart('sa-daily-trend-chart', data);
      },
      error: () => this.showEmptyState('sa-daily-trend-chart', 'No submission data available')
    });
  }

  private loadDemandByManagersChart(): void {
    const dateRange = this.getDateRange(this.demandManagersView, this.demandManagersMonth);
    this.superAdminService.getDemandByManagers(dateRange.startDate, dateRange.endDate).subscribe({
      next: (res: any) => {
        const data = res.distribution || [];
        this.renderPieChart('sa-demand-managers-chart', data, 'manager_name', 'count', 'percentage', true);
      },
      error: () => this.showEmptyState('sa-demand-managers-chart', 'No demand data available')
    });
  }

  private loadDemandByStatusChart(): void {
    const dateRange = this.getDateRange(this.demandStatusView, this.demandStatusMonth);
    this.superAdminService.getDemandByStatus(dateRange.startDate, dateRange.endDate).subscribe({
      next: (res: any) => {
        const data = res.status_counts || [];
        this.renderDonutChart('sa-demand-status-chart', data, 'status', 'count', 'percentage');
      },
      error: () => this.showEmptyState('sa-demand-status-chart', 'No status data available')
    });
  }

  private loadDemandBySkillChart(): void {
    const dateRange = this.getDateRange(this.demandSkillView, this.demandSkillMonth);
    this.superAdminService.getDemandBySkill(dateRange.startDate, dateRange.endDate).subscribe({
      next: (res: any) => {
        const data = res.skill_distribution || [];
        this.renderBarChart('sa-demand-skill-chart', data, 'skill', 'count', 'Skill', 'Demands');
      },
      error: () => this.showEmptyState('sa-demand-skill-chart', 'No skill data available')
    });
  }

  private loadSubmissionsByManagersChart(): void {
    const dateRange = this.getDateRange(this.submissionsManagersView, this.submissionsManagersMonth);
    this.superAdminService.getSubmissionsByManagers(dateRange.startDate, dateRange.endDate).subscribe({
      next: (res: any) => {
        const data = res.manager_submissions || [];
        this.renderBarChart('sa-submissions-managers-chart', data, 'manager_name', 'count', 'Manager', 'Submissions');
      },
      error: () => this.showEmptyState('sa-submissions-managers-chart', 'No submission data available')
    });
  }

  private loadDemandBySpocsChart(): void {
    const dateRange = this.getDateRange(this.demandSpocsView, this.demandSpocsMonth);
    this.superAdminService.getDemandBySpocs(dateRange.startDate, dateRange.endDate).subscribe({
      next: (res: any) => {
        const data = res.distribution || [];
        this.renderPieChart('sa-demand-spocs-chart', data, 'spoc_name', 'count', 'percentage', true);
      },
      error: () => this.showEmptyState('sa-demand-spocs-chart', 'No demand data available')
    });
  }
  ...
  private loadSubmissionsBySpocsChart(): void {
    ...
    this.superAdminService.getSubmissionsBySpocs(dateRange.startDate, dateRange.endDate).subscribe({
      next: (res: any) => {
        const data = res.spoc_submissions || [];
        this.renderBarChart('sa-submissions-spocs-chart', data, 'spoc_name', 'count', 'SPOC', 'Submissions');
      },
      error: () => this.showEmptyState('sa-submissions-spocs-chart', 'No submission data available')
    });
  }
  ...
  private renderLineChart(containerId: string, data: Array<{ date: string; count: number; managers?: Array<{ manager_name: string; count: number }> }>): void {
  ...
  private renderBarChart(containerId: string, data: any[], xKey: string, yKey: string, xLabel: string, yLabel: string): void {
    ...
    const getBarColor = (d: any, index: number): string => {
      ...
      if (containerId === 'sa-demand-status-chart' && this.statusColors[keyValue]) {
        return this.statusColors[keyValue];
      }
      
      // Use recruiterChartColors for submissions by managers chart
      if (containerId === 'sa-submissions-managers-chart') {
      ...
      if (containerId === 'sa-submissions-spocs-chart') {
        return this.recruiterChartColors[index % this.recruiterChartColors.length];
      }
